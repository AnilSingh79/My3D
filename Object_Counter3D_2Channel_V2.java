...
public class Object_Counter3D_2Channel_V2 implements PlugIn, AdjustmentListener, TextListener, Runnable {
	...
	//@@@@@@@@@@@
	   ...
	   ImageProcessor myIP = imgPlus.getProcessor();
	   for (int t=0; t!=contourList.size(); t++){
	       int x1  =  minx+(contourList.get(t)).x;
	       int y1  =  miny+(contourList.get(t)).y;
	       myIP.putPixel(x1,y1,0);
	       //System.out.println("object: "+i+", x: "+x1+", y :"+y1+", conc: "+contourList.get(t).concavescore);
	   }
	   ...
	   /*
		   //IJ.log("currentTotalObjects:"+currentTotalObjects);  
    	 ...
    	 else if(CenternumArray[i] >2)*/
    	 {
    		 ...
    		 //pass in an arraylist of VoxelPoints
    		 splitToTwo(i, CenterCoorArray.get(i));
    		 //consecutive split?...
     	 }
       }
    	 
	   currentTotalObjects = ID; //ID will change after calling splitToTwo.
	   for(i=0; i < currentTotalObjects; i++) 
	       {   
		   //@@@@@@@@@@
		   Tuple3D minLimits = limitsXYZ(i, "min");
		   ...
		   ImageProcessor myIP = img1.getProcessor();
		   for (int t=0; t!=contourList.size(); t++){
		       int x1  =  minx+(contourList.get(t)).x;
		       int y1  =  miny+(contourList.get(t)).y;
		       myIP.putPixel(x1,y1,0);
     ...
     // two centers detected from model. adjust by 1 for index.
	   /*
	 ...
	 IJ.log("number of object after splitting:"+ ID);   
      */
	...
	//(Paramarray[index][0]>=minSize && Paramarray[index][0]<=maxSize)
                                col=IDarray[index]+1;
                                ip.setValue(col);
                                ip.drawPixel(x, y);
                            }
    ...
    void splitToTwo(int objectIndex, ArrayList<VoxelPoint> voxelpoints)
    {
		...
		//IJ.log("centers:"+center1+" "+center2 + " center1.z:" +center1.z);
		splitToTwo(objectIndex, center1.x, center1.y, center1.z, center2.x, center2.y, center2.z);
	    }
 ...
 //---
    ...
    { 
	//	System.out.println("Splitting Object :"+objectIndex+", Num of centers: " + voxelpoints.size());
    ...
    {
	//	System.out.print("Num Points: "+voxelpoints.size());
	...
	int obwidth = maxx-minx+1, obheight = maxy-miny+1, obdepth = maxz-minz+1;
       ...
       if (obwidth*obheight*obdepth < minSize) return;  //too small to split
       ...
       //sort to get the top 2 (x1,  y1, x2, y2)
       ...
       //       boolean onContour(Tuple3D point, ArrayList<ContourPoint> cont)
       for(int contourindex = 0; contourindex < contourList.size()/2; contourindex++){
	   ContourPoint cp = contourList.get(contourindex);
	   int cpX = cp.x;
	   int cpY = cp.y;
	   for(int m= cpX-1; m<=cpX+1; m++){
	         for(int n= cpY-1; n<=cpY+1; n++){
		     ContourPoint neigh = new ContourPoint(m,n,0);
		     if (on
	   }
       }
       */
       ...
       //  System.out.println("First Point Concavity: "+ contourList.get(0).concavescore);
       ...
       int px,py;
       int iStore=i;
       while(mindist>900 && i<contourList.size()-1){
	    y1 = contourList.get(i).y;
	    x1 = contourList.get(i).x;
	    for(int contourindex = i+1; contourindex < contourList.size(); contourindex++)
		{
	       //get a line between
	         ...
	         if ((cy1-a*cx1-b)*(cy2-a*cx2-b)  < 0) //on two different side
		   {
		       ...
		       for (VoxelPoint p : voxelpoints)
			   {
			       ...
			       //System.out.println("Printing: "+(p.y-a*p.x-b));
       ...
       // System.out.println("Second Point Concavity: "+contourList.get(counter).concavescore+" Distance: "+mindist);
       int highestConcav = secondConcav;
       int newPoint = counter;
       x1 = contourList.get(iStore).x;
       y1 = contourList.get(iStore).y;
       for(int ii=-5; ii!=6;ii++){
	   if(counter+ii < contourList.size() && counter+ii >-1){
	       int xx2 = contourList.get(counter+ii).x;
	       int yy2 = contourList.get(counter+ii).y;
	       int concav = contourList.get(counter+ii).concavescore;
	       if(xx2==x1) continue;
	       if(yy2==y1) continue;
	       float tempa =  (y1 - yy2)/ (float)(x1-xx2);
	       float tempb =   y1 - tempa*x1;
	       if(!checkPath(tempa,tempb,cx1,cy1,voxelpoints))continue;
	       if(concav>highestConcav){
		   //	   newPoint=counter+ii;
		   a=tempa;
		   b=tempb;
	       }
       ...
       // System.out.println("OldPoint: "+counter+" Concavity: "+contourList.get(counter).concavescore);
       // System.out.println("NewPoint: "+newPoint+" Concavity: "+contourList.get(newPoint).concavescore);
       // // System.out.println("====");
       // // //Check this out.
       // x1 = contourList.get(iStore).x;
       // y1 = contourList.get(iStore).y;
       
       //  if(x1!=contourList.get(newPoint).x){
       // 	    x2 = contourList.get(newPoint).x;
       // 	    y2 = contourList.get(newPoint).y;

       // 	   a =  (y1 - y2)/ (float)(x1-x2);
       // 	   b =   y1 - a*x1;
       // 	}

       System.out.println("founda: "+a);
       System.out.println("foundb: "+b);
       System.out.println("found: "+founda);
       System.out.println("found: "+foundb);

       
       founda=a;
       foundb=b;
       //System.out.println("Concavity: "+contourList.get(counter+ii).concavescore+" Distance: "+ii);
       ...
       /* if (a==0 && b==0)
	  {
		...
		for(int y =0; y < obheight; y++)
				...
				{
				    //3/11/2014 Decide that the side containing cx1, cy1, cz1 will always have the incremented obj tag.  The other contains the original obj tag. 
  ...
  int splitToTwoLike(int objectIndex, int cx1, int cy1, int cz1, int cx2, int cy2, int cz2)
    {
        int index,arrayIndex;
	Tuple3D minLimits = limitsXYZ(objectIndex, "min");
	Tuple3D maxLimits = limitsXYZ(objectIndex, "max");
	...
	int obwidth = maxx-minx+1, obheight = maxy-miny+1, obdepth = maxz-minz+1;
       ...
       if (obwidth*obheight*obdepth < minSize) return 0;  //too small to split
       ...
       if(contourList.size() < 2){
	   IJ.log("Not enough contour points for this object: "+objectIndex+ "? Why?");
	   return 0;  //not enough contour points?
       }
       ...
       */
       ...
       int counter=1;
       for(int contourindex = 1; contourindex < contourList.size()/2; contourindex++)
	   //for(int contourindex = 1; contourindex < contourList.size(); contourindex++)
	   {
	       //get a line between
	       x2 = contourList.get(contourindex).x;
	       y2 = contourList.get(contourindex).y;
       ...
       return( (contourList.get(counter).concavescore)+(contourList.get(0).concavescore));
...
class VoxelPoint
{
	...
	for(VoxelPoint p : list)
	    {
		// distance += Object_Counter3D_2Channel_V2.getAbsDist(x, p.x, y, p.y);
		// if (z > p.z)  
		//     {
		// 	distance += z- p.z;
		//     }
